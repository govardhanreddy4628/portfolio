            ...
            <div className="w-full max-w-7xl mx-auto flex flex-col md:flex-row items-center md:items-center justify-between gap-16 md:gap-10 relative z-10">
                ...
                <div className="flex-[1.3] text-center md:text-left space-y-8">
                    ...
                    <div className="space-y-4">
                        ...
                        {/* Animated Title */}
                        <h1
                            className={`text-5xl md:text-7xl font-bold bg-gradient-to-r from-foreground to-primary bg-clip-text text-transparent transition-opacity duration-500 whitespace-nowrap ${fade ? "opacity-100" : "opacity-0"
                                }`}
                        >
                            {titles[index]}
                        </h1>
                ...
                <div className="flex-1 flex justify-center md:justify-end">
                    <div className="relative w-72 h-72 md:w-[380px] md:h-[380px] group">

                        {/* Outer Glow */}
                        <div className="absolute inset-0 bg-primary/20 blur-[60px] rounded-full opacity-70 group-hover:opacity-90 transition-all duration-500"></div>

                        {/* Floating Orb */}
                        <div className="absolute inset-0 rounded-full bg-gradient-to-br from-primary/30 via-purple-500/20 to-primary/10 blur-2xl scale-110 animate-slowFloat" />

                        {/* Concentric Rings */}
                        <svg viewBox="0 0 200 200" className="w-full h-full absolute inset-0 scale-110">
                            <circle
                                cx="100" cy="100" r="88"
                                stroke="rgba(99,102,241,0.22)" strokeWidth="6" fill="transparent"
                                className="group-hover:stroke-primary/40 transition-all duration-500"
                            />
                            <circle
                                cx="100" cy="100" r="62"
                                stroke="rgba(99,102,241,0.12)" strokeWidth="4" fill="transparent"
                                className="group-hover:stroke-primary/30 transition-all duration-500"
                            />
                        </svg>

                        {/* Profile Image */}
                        <div className="relative w-full h-full rounded-full overflow-hidden shadow-2xl border border-primary/20 group-hover:border-primary/40 transition-all duration-500">
                            <img
                                src={profileImage}
                                alt="Govardhan"
